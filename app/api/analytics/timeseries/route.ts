...
import { getAnalyticsTimeSeries, parseAnalyticsFilters } from "@/lib/analytics/service";
...
export async function GET(request: Request) {
  try {
    ...
    const timeSeries = await getAnalyticsTimeSeries(filters);

    return NextResponse.json({ timeSeries });
  } catch (error) {
    console.error("Failed to fetch analytics time series", error);

    if (error instanceof Error && error.message.startsWith("Invalid analytics filters")) {
      return NextResponse.json({ error: error.message }, { status: 400 });
    }

    return NextResponse.json({ error: "Failed to fetch analytics time series" }, { status: 500 });
  }
}
